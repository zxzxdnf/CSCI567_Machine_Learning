...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for j in range(max_iterations):
        ...
        for i in range(N):
            #B = w.dot(X[i].T)+b
            #B = B + np.min(B)
            #Sw = Sw + (sigmoid(B) - y[i]) * X[i]
            #Sb = Sb + sigmoid(B) - y[i]
            Sw = Sw + ((1 / (1 + np.exp( - (w.dot(X[i].T) + b)))) - y[i]) * X[i]
            Sb = Sb + (1 / (1 + np.exp( - (w.dot(X[i].T) + b)))) - y[i]
        
        w = w - step_size * Sw / N
        b = b - step_size * Sb / N
    '''
    for j in range(max_iterations):
        Sw = (sigmoid(w.dot(X.T) + b) - y).dot(X) / N
        Sb = sum((sigmoid(w.dot(X.T) + b) - y)) / N
        w = w - step_size * Sw
        b = b - step_size * Sb
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for k in range(C):
        ...
        w[k],b[k] = binary_train(X, y_k, w0, b0, step_size, max_iterations)
